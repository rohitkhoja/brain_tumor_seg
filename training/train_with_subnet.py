...
optimizer = torch.optim.Adam(concat_generators(model.parameters(), subnet.parameters()), lr=0.007)
...
for epoch in range(num_epochs):
    ...
    for i, (images, masks) in tqdm.tqdm(enumerate(train_dataloader), total=len(train_dataloader), desc=f"Epoch {epoch+1}", leave=False):
        ...
        subnet_weights = subnet(bottleneck)
        # mean of the subnet weights
        w_1 =subnet_weights[:,0,0,0].mean()
        w_2 =subnet_weights[:,1,0,0].mean()
        w_3 =subnet_weights[:,2,0,0].mean()
        loss = criterion(pred_mask, masks, w_1, w_2, w_3) + lambda_tv(1-w_1) + lambda_ce(1-w_2) + lambda_dice(1-w_3)

        # w_1 =subnet_weights[:,0,0,0]
        # w_2 =subnet_weights[:,1,0,0]
        # w_3 =subnet_weights[:,2,0,0]

        # loss = 0
        # for b in range(images.shape[0]):
        #     loss += criterion(pred_mask[b, :, :, :].unsqueeze(0), masks[b, :, :, :].unsqueeze(0), w_1[b], w_2[b], w_3[b]) + lambda_tv*(1-w_1[b]) + lambda_ce*(1-w_2[b]) + lambda_dice*(1-w_3[b])
        # loss /= images.shape[0]
